...
describe('Puppeteer testing suite', () => {
    ...
    before(async () => {
        ...
        m = require('mongodb').MongoClient.connect(uri, {
            ...
            .then(() => mc.db(db).collection(collection).deleteMany({}))
        ...
        await Promise.all([m, b]); // waiting for both promises

        m = await m;
        b = await b;
        
        expect(await mc.db(db).collection(collection).count()).to.be.equal(0);
    })
    

    after(() => {
        Promise.all([mc.close(), b.close()]);
    })
