...
const {db, collection, username, password, apiKey} = require(`./db-test.json`);
...
describe('Puppeteer testing suite', () => {
    ...
    before(async () => {
        ...
        var ret = await Promise.all([m, b]); // waiting for both promises

        m = await m;
        b = await b;
        
        expect(await mc.db(db).collection(collection).countDocuments()).to.be.equal(0);
        return ret;
    })
    

    after(() => {
        return Promise.all([mc.close(), b.close()]);
    });
    
    describe('Crawl',  async () => {
        it.only('Should crawl all domains on google', async () => {
            let companies = await main.crawlGoogleResults('https://www.google.com.ua/search?num=100&q=site%3Aui.ua+-inurl%3Adom-maister', b);
        ...
        it('Should crawl single company', async () => {
            let company = await main.crawlCompany('http://dom-maister.ui.ua', b);
